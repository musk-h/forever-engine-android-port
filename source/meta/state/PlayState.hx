...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (dialogueBox != null && dialogueBox.alive)
		{
			...
			#if android
			if (HSys.androidTouched() && dialogueBox.textStarted)
			#else
		...
		if (!inCutscene)
		{
			...
			if (FlxG.keys.justPressed.ENTER || HSys.androidBack()  && startedCountdown && canPause)
			{
				pauseGame();
			}
	...
	function callTextbox()
	{
		...
		if (HSys.exists(dialogPath))
		{
			startedCountdown = false;

			dialogueBox = DialogueBox.createDialogue(HSys.getContent(dialogPath));
